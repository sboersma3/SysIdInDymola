function PlotResultsPerBatch(sys,syshat,signals,SimuResults,SysIdResults,ops,kk)
...
figure(4);clf
subplot(2,1,1)
plot(w,syshat{kk*ops.ll*ops.Nb}.Aopt,'ro','linewidth',3);hold on;
plot(wi(kk,:),syshat{kk*ops.ll*ops.Nb}.Ai,'bo','linewidth',3);grid;
...
subplot(2,1,2)
plot(diag(syshat{kk*ops.ll*ops.Nb}.Phat(syshat{kk*ops.ll*ops.Nb}.CritPar,syshat{kk*ops.ll*ops.Nb}.CritPar)),'ro','linewidth',3);hold on;
...
set(gca,'ylim',[0,1.2*max(max(diag(syshat{kk*ops.ll*ops.Nb}.P(syshat{kk*ops.ll*ops.Nb}.CritPar,syshat{kk*ops.ll*ops.Nb}.CritPar)),...
    diag(syshat{kk*ops.ll*ops.Nb}.Phat(syshat{kk*ops.ll*ops.Nb}.CritPar,syshat{kk*ops.ll*ops.Nb}.CritPar))))])
