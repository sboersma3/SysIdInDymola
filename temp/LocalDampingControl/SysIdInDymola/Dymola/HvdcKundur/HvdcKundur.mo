package HvdcKundur
  model NetworkSimu
    ...
    Modelica.Electrical.Analog.Basic.Capacitor capacitor1(C = 195E-6, v(fixed = true, start = 400E3)) annotation (
      Placement(visible = true, transformation(origin={-50,104},    extent = {{-10, -10}, {10, 10}}, rotation = -90)));
    Modelica.Electrical.Analog.Basic.Capacitor capacitor2(C = 195E-6, v(fixed = true, start = 400E3)) annotation (
      Placement(visible = true, transformation(origin={38,102},    extent = {{-10, -10}, {10, 10}}, rotation = -90)));
    ...
    Modelica.Blocks.Interfaces.RealOutput u
      annotation (Placement(transformation(extent={{-214,82},{-194,102}})));
    Modelica.Blocks.Math.Add add1(k2 = -1) annotation (
      Placement(transformation(extent={{-116,-48},{-104,-36}})));
  ...
  HVDCcomponents.VSC_station_dq0_with_control_PLL_bis vSC_station_dq0_with_control_PLL_bis1(Lr = 0.05, P_ref = 0, Q_ref = 0, Rr = 0.5, Vgd = PF_results.voltages.V7 * 400E3, teta_bus = PF_results.voltages.A7 * Modelica.Constants.pi / 180, tr = 0.003, wg = 2 * Modelica.Constants.pi * 60)  annotation (
      Placement(visible = true, transformation(origin={-94,102},    extent = {{40, -13}, {-20, 12}}, rotation = 0)));
  HVDCcomponents.VSC_station_dq0_with_control_PLL_bis vSC_station_dq0_with_control_PLL_bis2(Lr = 0.05, P_ref = 0, Q_ref = 0, Rr = 0.5, Vgd = PF_results.voltages.V9 * 400E3, teta_bus = PF_results.voltages.A9 * Modelica.Constants.pi / 180, tr = 0.003, wg = 2 * Modelica.Constants.pi * 60)  annotation (
    ...
    ExcitationGeneration.Multisine_Noise_Simu multisine_Noise_Simu annotation (
      Placement(transformation(extent={{-262,64},{-242,84}})));
    Modelica.Blocks.Sources.Constant const1(k=10)  annotation (
      Placement(transformation(extent={{-260,90},{-254,96}})));
    Modelica.Blocks.Sources.Constant const2(k=5)   annotation (
      Placement(transformation(extent={{-262,100},{-256,106}})));
    ExcitationGeneration.Ground_phasor ground_phasor annotation (
      Placement(transformation(extent={{-238,72},{-234,76}})));
    ...
    Modelica.Blocks.Sources.Step step1(height=0, startTime=2)     annotation (
      Placement(visible = true, transformation(origin={-100,130},   extent = {{-6, -6}, {6, 6}}, rotation = 0)));
  equation
    ...
    connect(capacitor2.n, vSC_station_dq0_with_control_PLL_bis2.pin_n) annotation (
      Line(points={{38,92},{62,92},{62,106},{75.2,106},{75.2,105.6}},      color = {0, 0, 255}));
    connect(capacitor2.p, vSC_station_dq0_with_control_PLL_bis2.pin_p) annotation (
      Line(points={{38,112},{56,112},{56,110},{75,110},{75,109.8}},          color = {0, 0, 255}));
    connect(step2.y, vSC_station_dq0_with_control_PLL_bis1.Qref) annotation (
      Line(points={{-43.4,174},{-32,174},{-32,126},{-68,126},{-68,106},{-62.5,
            106},{-62.5,105.9}},                                                                          color = {0, 0, 127}));
    connect(capacitor1.n, vSC_station_dq0_with_control_PLL_bis1.pin_n) annotation (
      Line(points={{-50,94},{-68,94},{-68,96},{-63.2,96},{-63.2,95.6}},      color = {0, 0, 255}));
    connect(capacitor1.p, vSC_station_dq0_with_control_PLL_bis1.pin_p) annotation (
      Line(points={{-50,114},{-64,114},{-64,100},{-63,100},{-63,99.8}},           color = {0, 0, 255}));
    connect(vSC_station_dq0_with_control_PLL_bis1.p, pQ_sensor2.p) annotation (
      Line(points={{-85,101.8},{-126,101.8},{-126,98},{-146.4,98}},                 color = {0, 0, 255}));
    ...
    connect(voltageSensor1.n, capacitor2.n) annotation (
      Line(points={{48,148},{50,148},{50,92},{38,92}},                    color = {0, 0, 255}));
    connect(voltageSensor1.p, capacitor2.p) annotation (
      Line(points={{28,148},{22,148},{22,122},{36,122},{36,112},{38,112}},              color = {0, 0, 255}));
    connect(ground1.p, capacitor2.n) annotation (
      Line(points={{20,86},{38,86},{38,92}},                  color = {0, 0, 255}));
    connect(capacitor2.n, dC_cable1.pin_n1) annotation (
      Line(points={{38,92},{14,92},{14,104},{4.4,104}},                      color = {0, 0, 255}));
    connect(dC_cable1.pin_p1, capacitor2.p) annotation (
      Line(points={{4.4,111.8},{36.4,111.8},{36.4,112},{38,112}},          color = {0, 0, 255}));
    connect(dC_cable1.pin_n, capacitor1.n) annotation (
      Line(points={{-16.3,103.9},{-32.3,103.9},{-32.3,93.9},{-50,93.9},{-50,94}},            color = {0, 0, 255}));
    connect(capacitor1.p, dC_cable1.pin_p) annotation (
      Line(points={{-50,114},{-22,114},{-22,112},{-16.4,112}},                        color = {0, 0, 255}));
    ...
    connect(pQ_sensor2.n, bus7.p) annotation (
      Line(points={{-153.6,98},{-160,98},{-160,56},{-58,56},{-58,30}},            color = {0, 0, 255}));
    ...
    connect(pQ_sensor1.n, bus9.p) annotation (
      Line(points={{141.6,98},{162,98},{162,56},{58,56},{58,30}},            color = {0, 0, 255}));
    ...
    connect(const1.y, multisine_Noise_Simu.vr) annotation (Line(points={{-253.7,
            93},{-248.4,93},{-248.4,84}}, color={0,0,127}));
    connect(const2.y,multisine_Noise_Simu. vi) annotation (
      Line(points={{-255.7,103},{-245.6,103},{-245.6,84}},         color = {0, 0, 127}));
    connect(multisine_Noise_Simu.pwPin,ground_phasor. p) annotation (
      Line(points={{-243,77},{-236,77},{-236,75.96}},           color = {0, 0, 255}));
    connect(bus1.p, Line5_1.p)
      annotation (Line(points={{-184,30},{-169,30}}, color={0,0,255}));
    connect(g1.pwPin, bus1.p)
      annotation (Line(points={{-201.4,30},{-184,30}}, color={0,0,255}));
    connect(bus1.Angle, add1.u1) annotation (Line(points={{-174.6,35},{-132,35},
            {-132,-38.4},{-117.2,-38.4}}, color={0,0,127}));
    connect(add1.u2, bus2.Angle) annotation (Line(points={{-117.2,-45.6},{
            -117.2,-46},{-138,-46},{-138,7.2},{-174,7.2}}, color={0,0,127}));
    connect(step1.y, vSC_station_dq0_with_control_PLL_bis1.Pref) annotation (
        Line(points={{-93.4,130},{-78,130},{-78,110.5},{-62.5,110.5}}, color={0,
            0,127}));
    connect(y, add1.y)
      annotation (Line(points={{-82,-42},{-103.4,-42}}, color={0,0,127}));
    connect(u, multisine_Noise_Simu.y1_u) annotation (Line(points={{-204,92},{
            -232,92},{-232,69},{-243,69}}, color={0,0,127}));
    connect(g1.u, multisine_Noise_Simu.y1_u) annotation (Line(points={{-215.2,
            34.92},{-232,34.92},{-232,69},{-243,69}}, color={0,0,127}));
    annotation (
...
<td><p><a href=\"mailto:luigiv@kth.se\">luigiv@kth.se</a></p></td>
  ...
  model NetworkSysId
    ...
    Modelica.Blocks.Interfaces.RealOutput y1 annotation (
      Placement(transformation(extent={{-114,-50},{-94,-30}})));
    Modelica.Blocks.Math.Add add1(k2 = -1) annotation (
      Placement(transformation(extent={{-138,-46},{-126,-34}})));
    Modelica.Blocks.Interfaces.RealInput P_ref annotation (
      Placement(transformation(extent={{-284,14},{-244,54}})));
    HVDCcomponents.PQ_sensor pQ_sensor3(S_b = 100) annotation (
      Placement(visible = true, transformation(origin = {-46, 30}, extent = {{-3.6, -3}, {3.6, 3}}, rotation = 180)));
  HVDCcomponents.VSC_station_dq0_with_control_PLL_bis vSC_station_dq0_with_control_PLL_bis1(Lr = 0.05, P_ref = 0, Q_ref = 0, Rr = 0.5, Vgd = PF_results.voltages.V7 * 400E3, teta_bus = PF_results.voltages.A7 * Modelica.Constants.pi / 180, tr = 0.003, wg = 2 * Modelica.Constants.pi * 60)  annotation (
  ...
  equation
    ...
    connect(add1.y, y1) annotation (
      Line(points={{-125.4,-40},{-104,-40}},color = {0, 0, 127}));
    connect(bus7.p, pQ_sensor3.n) annotation (
    ...
    connect(step1.y, vSC_station_dq0_with_control_PLL_bis1.Pref) annotation (
        Line(points={{-93.4,130},{-84,130},{-84,112.5},{-64.5,112.5}}, color={0,
            0,127}));
    connect(bus2.p, g2.pwPin)
      annotation (Line(points={{-180,0},{-201.4,0}}, color={0,0,255}));
    connect(bus2.p, Line5_2.p)
      annotation (Line(points={{-180,0},{-169,0}}, color={0,0,255}));
    connect(Line5_1.p, bus1.p)
      annotation (Line(points={{-169,30},{-180,30}}, color={0,0,255}));
    connect(g1.pwPin, bus1.p)
      annotation (Line(points={{-201.4,30},{-180,30}}, color={0,0,255}));
    connect(add1.u1, bus1.Angle) annotation (Line(points={{-139.2,-36.4},{-146,
            -36.4},{-146,35},{-170.6,35}}, color={0,0,127}));
    connect(add1.u2, bus2.Angle) annotation (Line(points={{-139.2,-43.6},{-150,
            -43.6},{-150,5},{-170.6,5}}, color={0,0,127}));
    connect(P_ref, g1.u) annotation (Line(points={{-264,34},{-240,34},{-240,
            34.92},{-215.2,34.92}}, color={0,0,127}));
    annotation (
      Diagram(coordinateSystem(preserveAspectRatio = false, extent = {{-240, -80}, {220, 220}}), graphics={  Text(lineColor = {28, 108, 200}, extent = {{-72, -66}, {50, -74}}, textString = "Two-Area System
Prabha Kundur, \"Power System Stability and Control\", Example 12.6, page 813")}),
      Icon(coordinateSystem(extent = {{-240, -80}, {220, 220}}, preserveAspectRatio = false)),
