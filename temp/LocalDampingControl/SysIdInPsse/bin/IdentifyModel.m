...
optOE  = oeOptions('Focus','prediction','InitialCondition' ,'auto','EnforceStability',true);
for kk=1:floor(N/Nb)
    t1                          = (kk-1)*Nid+1;
    t2                          = kk*Nid;
    
    syshat{kk*ll*Nb}.sys        = oe(iddata(y(t1:t2)',u(t1:t2)',h),[nb nf nk],optOE);

    [~,numG,~,~,den]            = polydata(syshat{kk*ll*Nb}.sys);
    numH                        = den;
    syshat{kk*ll*Nb}.den        = den;
    syshat{kk*ll*Nb}.numG       = numG;
    syshat{kk*ll*Nb}.numH       = numH;
    syshat{kk*ll*Nb}.G          = ss(tf(numG,den,h));
    syshat{kk*ll*Nb}.H          = ss(minreal(tf(numH,den,h)));
