...
function phiucorr=optphi(begincout,barM,M0,vecM,M,k,ndata,w,na,nb,delay,size,C,deltastar,Wl1tf,Wl2tf,Wr1tf,Wr2tf,phiumax,phiymax,vare,G,H,openloop)
...
lmiterm([counter,1,1,sigma],vecs,1);
lmiterm([counter,1,1,0],vecconstpouru-phiumax(jjj));
if M>0
    veccos=[];
    for lll=1:M,
    ...
    lmiterm([counter,1,1,corrvec],veccos2,1);
end;

counter=counter+1;

lmiterm([counter,1,1,sigma],vecgs,1);
lmiterm([counter,1,1,0],vecconstpoury-phiymax(jjj));
if M>0
    
    veccos3=2*vecgs*veccos;
    lmiterm([counter,1,1,corrvec],veccos3,1);
end;
...
vecg=abs(frxav(G,w(jjj)))^2;
vech=abs(frxav(H,w(jjj)))^2;



vecconstpoury=vech*vare;

lmiterm([counter,1,1,sigma],1,1);
lmiterm([counter,1,1,0],-phiumax(jjj));
if M>0
    veccos=[];
    for lll=1:M,
        veccos=[veccos,cos(lll*w(jjj))];
...
lmiterm([counter,1,1,sigma],vecg,1);
lmiterm([counter,1,1,0],vecconstpoury-phiymax(jjj));
if M>0
    
    veccos3=2*vecg*veccos;
    lmiterm([counter,1,1,corrvec],veccos3,1);
end;
end;
