...
ops.w               = linspace(.1*2*pi,20*2*pi,25);
ops.Nw              = length(ops.w);
...
ubase     = 0;
for ll=1:ops.Nw
    ubase = ubase + ops.Ai(ll)*sin(ops.w(ll)*t)';
end
...
u1     = 0;
for ll=1:ops.Nw
    u1 = u1 + ops.Ai(ll)*sin(ops.w(ll)*t1)';
end
...
for ll=1:ops.Nw
    ...
    u2            = u2 + ops.Aiopt(ll)*sin(ops.w(ll)*t1)';
end
...
subplot(2,3,4)
plot(ops.w,ops.Aiopt,'ro','linewidth',3);grid;
xlabel('$\omega$ (rad/s)','interpreter','latex');
ylabel('$A_i$','interpreter','latex');
ylim([0 1.1*max(ops.Aiopt)])
subplot(2,3,5)
...
%% Check if estimation is unbiased and check variance expression
clear;clc
...
t                   = 0:h:20-h;
ops.Nid             = length(t);
ops.w               = linspace(.1*2*pi,5*2*pi,30);      
ops.wlog            = logspace(log10(0.1),log10(5),30);
ops.Nw              = length(ops.w);

% define true system
z                   = tf('z',h);

temp                = load(strcat('C:\Users\Sjoerd\Desktop\Dymola\results\SingleMachineInfiniteBus\',"LinRes",num2str(10),'.mat'));
...
% den0                = den0(1:8);
% numG0               = numG0(1:8);
% numH0               = numH0(1:8)/numH0(1); % adapt the linearized model...
% 
% G0                  = tf(numG0,den0,h);
% H0                  = tf(numH0,den0,h);
...
clear temp directory G zz den D zetahati wnhati zetahatr wnhatr pi pr kk ll G;
...
for kk=1:20
    ...
    u           = 0;%sigma_u*randn(ops.Nid,1);
    for ll=1:ops.Nw
        u = u + ops.Ai(ll)*sin(ops.w(ll)*t+randn(length(t),1))';
    end
...
% covariance matrix base case
Pbase     = inv(Mbar + M * kron(ops.Ai'.^2,eye(nx)));
Ibase     = Mbar + M * kron(ops.Ai'.^2,eye(nx));
ubase     = 0;
for ll=1:ops.Nw
    ubase = ubase + ops.Ai(ll)*sin(ops.w(ll)*t)';
end
...
% covariance matrix
P1     = inv(Mbar + M * kron(ops.Ai'.^2,eye(nx)));
I1     = Mbar + M * kron(ops.Ai'.^2,eye(nx));
u1     = 0;
for ll=1:ops.Nw
    u1 = u1 + ops.Ai(ll)*sin(ops.w(ll)*t1)';
end
...
ops.c2                 = (1-ops.c1);                         % weithing factor for output
ops.alpha              = P1(syshat.CritPar,syshat.CritPar) - Pbase(syshat.CritPar,syshat.CritPar);
%ops.alpha              = inv(Ibase(syshat.CritPar,syshat.CritPar) - I1(syshat.CritPar,syshat.CritPar));
[PHIopt,vecC,C,P2]  = get_optimal_spectrum_input(syshat,ops);
...
% Iopt                = I1+I2;
% Ibase(syshat.CritPar,syshat.CritPar)
% Iopt(syshat.CritPar,syshat.CritPar)
...
figure(1);clf;
subplot(2,3,1)
semilogx(ops.w,ops.Ai,'ro','linewidth',3);grid;
xlabel('$\omega$ (rad/s)','interpreter','latex');
ylabel('$A_i$','interpreter','latex');
subplot(2,3,2)
plot(t,ubase,'r');grid
ylabel('$u_{base}$','interpreter','latex');
title(['$P_u$ = ' ,num2str(ubase'*ubase,4)],'interpreter','latex'); 
subplot(2,3,3)
plot(t,ybase);grid
ylabel('$y_{base}$','interpreter','latex');
title(['$P_y$ = ' ,num2str(ybase'*ybase,4)],'interpreter','latex');
subplot(2,3,4)
semilogx(ops.w,ops.Aiopt,'ro','linewidth',3);grid;
xlabel('$\omega$ (rad/s)','interpreter','latex');
ylabel('$A_i$','interpreter','latex');
subplot(2,3,5)
plot(t,uopt,'r');grid
ylabel('$u_{opt}$','interpreter','latex');
title(['$P_u$ = ' ,num2str(uopt'*uopt,4)],'interpreter','latex'); 
subplot(2,3,6)
plot(t,yopt);grid
ylabel('$y_{opt}$','interpreter','latex');
xlabel('$t$ (sec)','interpreter','latex');
title(['$P_y$ = ' ,num2str(yopt'*yopt,4)],'interpreter','latex')
...
%% set initial condition of optimal input
t    = 0:h:10;
Nid  = length(t);
uopt = 6000*sin(t);
...
U    = zeros(1,length(t));
for kk=1:ops.Nw
    U = U + A(kk)*cos(ops.w(kk)*t);
end

% force the excitation signal to start at correct initial condition 
[~,ind] = min( abs(U(1:length(t)-(ops.Nid-1))-u0) );

U       = U(ind:ind+ops.Nid-1);
...
plot(U'/1e3);grid
subplot(2,2,[3 4])
plot([uopt'/1e3;U'/1e3]);grid;hold on
