function [PHIopt,vecC,C,P] = get_optimal_spectrum_input(syshat,alpha,N,c1,c2)
...
M              = [];
for kk=1:Nw
    M = [M,(N/vare/2)*real(Fuw(:,:,kk)*Fuw(:,:,kk)')];
end
...
for kk=1:Np
    disp(' ')
    disp(['the variance and its upperbound are: ',num2str([diagP(ind(kk)) alpha(kk)])])
    disp(' ')
end
